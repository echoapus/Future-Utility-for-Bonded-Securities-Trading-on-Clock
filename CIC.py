...
def calculate_opening_momentum(symbol, reststock):
    ...
    try:
        ...
        five_min_price = data[4].get('close', 0) if len(data) >= 5 else data[-1].get('close', 0)
...
def screen_stocks(reststock):
    ...
    try:
        ...
        for i, stock in enumerate(filtered_stocks[:10], 1):
            print(f"{i:<4} {stock['symbol']:<8} {stock['name']:<12} {stock['volume']:>10,} {stock['change_pct']:>+6.2f}%")
        
        # 重新組織成原來的格式供後續分析
        stock_list = [{'symbol': s['symbol'], 'name': s['name']} for s in filtered_stocks]
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_stock):
            ...
            print(f"分析中... {processed}/{analyze_count} "
                  f"({processed/analyze_count*100:.1f}%) "
                  f"速度:{rate:.1f}檔/秒 預計剩餘:{eta:.0f}秒", end='\r')
...
def display_results(stocks):
    ...
    # 按分數排序 (總分 > 加分項 > 額外分)
    stocks.sort(key=lambda x: (x['total_score'], x['bonus_score'], x['extra_score']), reverse=True)
    
    print(f"\n篩選結果 (共 {len(stocks)} 檔)")
    print("=" * 110)
    print(f"{'代碼':<8} {'股名':<12} {'價格':<8} {'漲跌':<10} {'量(張)':<10} {'必要':<15} {'加分':<25} {'總分':<6}")
    print("-" * 110)
    ...
    for i, stock in enumerate(stocks[:20], 1):  # 顯示前20檔
        ...
        print(f"{symbol:<8} {name:<12} {price:<8} {change_text:<10} {volume:<10} {required_text:<15} {bonus_text:<25} {score_text:<6}")
    ...
    if stocks:
        ...
        if full_bonus:
            print(f"滿分推薦: " + 
                  ", ".join([f"{s['symbol']}({s['total_score']}分)" for s in full_bonus[:5]]))
        ...
        print(f"\n加分條件統計:")
        print(f"量比達標: {len(vol_ratio_pass)}檔 | 開盤動能: {len(momentum_pass)}檔 | VWAP乖離: {len(vwap_pass)}檔")
