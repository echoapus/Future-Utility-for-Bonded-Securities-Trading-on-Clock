...
def display_orders_preview(orders):
    ...
    for i, order in enumerate(orders, 1):
        ...
        total_amount += amount
        print(f"{i:<4} {order['code']:<8} {order['quantity']:<6} {order['price']:<10} {amount:>8,.0f}")
...
def place_single_order(order_data, index, total):
    ...
    try:
        ...
        print(f"⏳ [{index}/{total}] 處理中：{order_data['code']} {order_data['quantity']}股...")
        
        # 發送下單請求
        result = sdk.stock.place_order(account, order, unblock=False)
        
        if result.is_success and result.data:
            status_label = STATUS_MAP.get(result.data.status, f"狀態碼：{result.data.status}")
            print(f"✅ [{index}/{total}] 成功：{order_data['code']} - 委託書號：{result.data.order_no or '未回傳'} - {status_label}")
        ...
        with open(report_filename, 'w', encoding='utf-8') as f:
            ...
            if success_orders:
                ...
                for item in success_orders:
                    ...
                    status = STATUS_MAP.get(result.status, f"狀態碼：{result.status}") if result else "未知"
                    f.write(f"股票：{order['code']}, 股數：{order['quantity']}, 價格：{order['price']}, 委託書號：{order_no}, 狀態：{status}\n")
                f.write("\n")
            ...
            if failed_orders:
                ...
                for item in failed_orders:
                    order = item['order_data']
                    f.write(f"股票：{order['code']}, 股數：{order['quantity']}, 價格：{order['price']}, 錯誤：{item['error']}\n")
