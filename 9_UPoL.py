...
def generate_summary_report(data):
    """產生總結報告"""
    total_cost = sum(
        item.cost_price * item.tradable_qty
        for item in data
        if item.cost_price and item.tradable_qty
    )
...
def create_detailed_report(data):
    ...
    # 表頭
    lines.append(
        "股票代號 | 股票名稱     | 庫存量 | 平均成本 | 現價   | 獲利率    | 未實現損益"
    )
    lines.append("-" * 80)
    ...
    try:
        ...
        if result.is_success and result.data:
            ...
            for item in result.data:
                ...
                current_price = get_current_price(item.stock_no)
                profit_rate = calculate_profit_rate(item.cost_price, current_price)
                net_unrealized = (item.unrealized_profit or 0) - (
                    item.unrealized_loss or 0
                )

                profit_icon = (
                    "🟢" if net_unrealized > 0 else "🔴" if net_unrealized < 0 else "⚪"
                )

                print(f"{profit_icon} {item.stock_no} ({stock_name})")
                print(
                    f"   庫存：{item.tradable_qty:,} 股 | 成本：{item.cost_price:.2f} | 損益：{net_unrealized:+,.0f} ({profit_rate:+.2f}%)"
                )
                print()
            ...
            print(
                f"   📈 淨損益：{summary['net_profit']:+,.0f} 元 ({summary['profit_rate']:+.2f}%)"
            )

        else:
            print(
                "❌ 查詢失敗：",
                result.message if hasattr(result, "message") else "未知錯誤",
            )
