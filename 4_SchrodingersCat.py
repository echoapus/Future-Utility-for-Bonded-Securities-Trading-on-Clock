...
def get_stock_cost_info(item):
    ...
    if hasattr(item, "odd") and item.odd:
        odd_buy = item.odd.buy_value if item.odd.buy_value and item.odd.buy_value > 0 else 0
        odd_sell = item.odd.sell_value if item.odd.sell_value and item.odd.sell_value > 0 else 0
    
    # 估算平均成本
    total_cost = regular_buy + odd_buy
    total_qty = item.today_qty + (item.odd.today_qty if hasattr(item, "odd") and item.odd else 0)
...
def format_enhanced_inventory(data):
    ...
    lines.append("")
    lines.append("📋 投資組合統計：")
    lines.append(f"   持股檔數：{len([i for i in data if (i.today_qty + (i.odd.today_qty if hasattr(i, 'odd') and i.odd else 0)) > 0])} 檔")
...
def show_cost_analysis(data):
    ...
    if stocks_with_cost:
        ...
        for stock in sorted(stocks_with_cost, key=lambda x: x['cost'], reverse=True)[:5]:
            print(f"      {stock['stock_no']}: {stock['qty']} 股, 成本 {stock['cost']:,.0f} 元 (平均 {stock['avg_cost']:.2f} 元/股)")
    
    print(f"   ❓ 無成本資料：{len(stocks_without_cost)} 檔")
    if stocks_without_cost:
        for stock in stocks_without_cost:
            print(f"      {stock['stock_no']}: {stock['qty']} 股 (可能為轉入或早期持股)")
...
def show_summary_stats(data):
    ...
    print(f"   整股合計：{total_regular_shares:,} 股 ({total_regular_shares/1000:.1f} 張)")
