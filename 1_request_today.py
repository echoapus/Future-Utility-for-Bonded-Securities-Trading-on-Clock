...
def show_filled_summary(data):
    ...
    for order in filled_orders:
        ...
        print(
            f"{icon} {order.stock_no} | {buy_sell_text} | "
...
def get_statistics(data):
    ...
    unfilled = len(
        [
            o
            for o in data
            if (o.filled_qty is None or o.filled_qty == 0) and o.status == 10
        ]
    )
    cancelled = len([o for o in data if o.status in [30, 40]])
    partial = len(
        [o for o in data if o.filled_qty is not None and 0 < o.filled_qty < o.quantity]
    )
...
def main():
    ...
    try:
        ...
        if not (result.is_success and result.data):
            print(
                "❌ 查詢失敗：",
                result.message if hasattr(result, "message") else "無資料",
            )
            return
