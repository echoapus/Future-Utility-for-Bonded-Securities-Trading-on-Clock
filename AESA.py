    ...
    try:
        ...
        if big_orders:
            ...
            if big_orders["bid_volume"] > 0 or big_orders["ask_volume"] > 0:
                print(
                    f"內盤:{big_orders['bid_volume']:,}張 外盤:{big_orders['ask_volume']:,}張"
                )
        ...
        if trades and trades.get("data"):
            ...
            for trade in trades["data"][:5]:
                t = time.strftime(
                    "%H:%M:%S", time.localtime(trade.get("time", 0) / 1000000)
                )
                size = trade.get("size", 0)
...
def main():
    ...
    # 同時等待用戶輸入
    print("請輸入股票代碼 (系統背景登入中):")
    code = input("股票代碼: ").strip().upper()
    ...
    if login_success:
        ...
        if code and code not in ["Q", "QUIT"]:
            analyze_stock_complete(reststock, code)
    else:
        print("登入失敗，無法查詢")
